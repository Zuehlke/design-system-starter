import {ColorItem, ColorPalette, Meta} from '@storybook/blocks';
import tokens from './design-tokens.json';
import {groupColors} from './color.helper';

<Meta title="Design Tokens" />

## Brand Colors

These colors show the base colors of the theme. In components, don't use these colors directly but prefer using the
abstract colors below. This allows for later color changes/theming/dark mode without having to rename a lot of
properties.

<ColorPalette>
  {Object.entries(groupColors(tokens.color.brand)).map(
    ([name, swatches], index) => (
      <ColorItem key={index} title={name} subtitle={""} colors={swatches} />
    )
  )}
</ColorPalette>

## Universal Colors

<ColorPalette>
  {Object.entries(groupColors(tokens.color.universal)).map(
    ([name, swatches], index) => (
      <ColorItem key={index} title={name} subtitle={""} colors={swatches} />
    )
  )}
</ColorPalette>

## Gradients

<ColorPalette>
  {Object.entries(groupColors(tokens.gradient)).map(
    ([name, swatches], index) => (
      <ColorItem key={index} title={name} subtitle={""} colors={swatches} />
    )
  )}
</ColorPalette>

## Typography

**Typeface:** {tokens.typography.main.fontFamily.value}

<div style={{ display: 'grid', gridTemplateColumns: 'max-content auto', gap: tokens.size['2'].value, alignItems: 'center' }}>
  {Object.entries(tokens.typography)
    .map(([name, token]) => (
      <div style={{ display: 'contents' }}>
        <small>{name}</small>
        <div
          style={{
            fontFamily: token.fontFamily.value,
            fontSize: token.fontSize.value,
            lineHeight: token.lineHeight.value,
            fontWeight: token.fontWeight.value,
            fontStretch: token.fontStretch.value,
            letterSpacing: token.letterSpacing.value,
            textDecorationStyle: token.textDecoration.value,
            paragraphIndent: token.paragraphIndent.value,
          }}
        >
          The quick brown fox jumps over the lazy dog
        </div>
      </div>
    ))
  }
</div>

## Typography

{/* **Typeface:** {tokens.typography.desktop.font.family.title.value} */}

{/* <div style={{ display: 'grid', gridTemplateColumns: 'max-content auto', gap: tokens.size['1'].value, alignItems: 'center' }}>
  {Object.entries(tokens.typography)
    .map(([name, token]) => (
      <div style={{ display: 'contents' }}>
        <small>{name}</small>
        <div
          style={{
            fontFamily: token.fontFamily.value,
            fontSize: token.fontSize.value,
            lineHeight: token.lineHeight.value,
            fontWeight: token.fontWeight.value,
            fontStretch: token.fontStretch.value,
            letterSpacing: token.letterSpacing.value,
            textDecorationStyle: token.textDecoration.value,
            paragraphIndent: token.paragraphIndent.value,
          }}
        >
          The quick brown fox jumps over the lazy dog
        </div>
      </div>
    ))
  }
</div> */}

{/* ## Sizes

<div
  style={{
    display: "grid",
    gridTemplateColumns: "min-content 5rem auto",
    gap: "var(--size-2)",
    alignItems: "center",
  }}
>
  {Object.entries(tokens.size)
    .sort(([, tokenA], [, tokenB]) => {
      if (tokenA.value.includes("px")) {
        return -1;
      }
      if (tokenB.value.includes("px")) {
        return 1;
      }
      const numberA = parseFloat(tokenA.value);
      const numberB = parseFloat(tokenB.value);
      return numberA - numberB;
    })
    .map(([, token], index) => (
      <div key={index} style={{display: 'contents'}}>
        <div
          style={{
            width: token.value,
            height: token.value,
            backgroundColor: "var(--color-brand-brand-100)",
          }}
        />
        <span>{token.value}</span>
        <strong>{token.name}</strong>
      </div>
    ))}
</div> */}

{/* ## Effects

<div
  style={{
    display: "grid",
    gridTemplateColumns: "min-content auto",
    gap: "var(size-4)",
    alignItems: "center",
  }}
>
  {Object.entries(tokens.effect).map(([name, token], index) => (
    <div key={index} style={{display: 'contents'}}>
      <div
        style={{
          boxShadow: token.value,
          height: "var(--size-6)",
          width: "var(--size-6)",
          borderRadius: "var(--radius-4)",
          backgroundColor: "var(--color-brand-brand-100)",
        }}
      />
      <strong>{token.name}</strong>
    </div>
  ))}
</div> */}

{/* ## Border radius

<div
  style={{
    display: "grid",
    gridTemplateColumns: "min-content min-content auto",
    gap: "var(size-4)",
    alignItems: "center",
  }}
>
  {Object.entries(tokens.radius).map(([, token], index) => (
    <div key={index} style={{display: 'contents'}}>
      <div
        style={{
          border: `var(--size-0-25) solid var(--color-brand-brand-100)`,
          height: "var(--size-6)",
          width: "var(--size-6)",
          borderRadius: token.value,
        }}
      />
      <span>{token.value}</span>
      <strong>{token.name}</strong>
    </div>
  ))}
</div> */}
